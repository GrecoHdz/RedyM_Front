...
export default defineNuxtPlugin(() => {
  ...
  const handleSWMessage = async (event: MessageEvent) => {
    ...
    console.log('📱 [PWA Push Handler] Navegación solicitada por SW hacia:', targetUrl)
    ...
    try {
      ...
      if (needsRefresh) {
        console.log('📱 [PWA Push Handler] Token vencido o ausente, intentando refresh...')
        const refreshed = await auth.refreshToken()
        if (!refreshed) {
          console.warn('📱 [PWA Push Handler] Refresh fallido, redirigiendo a login')
          await navigateTo('/', { replace: true })
          return
        }
        console.log('✅ [PWA Push Handler] Sesión restaurada')
      }

      // Navegar al destino de la notificación
      await navigateTo(targetUrl, { replace: true })
    } catch (e) {
      console.error('📱 [PWA Push Handler] Error al procesar navegación:', e)
