<template>
  <div class="min-h-screen bg-[#070b14] text-gray-100 font-['Outfit'] overflow-x-hidden selection:bg-emerald-500/30 selection:text-emerald-400">
    ...
    <!-- Header -->
    <header class="fixed top-0 w-full z-40 backdrop-blur-md border-b border-white/5 px-6 py-4">
      <div class="max-w-7xl mx-auto flex items-center justify-between">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 bg-gradient-to-tr from-emerald-500 to-teal-400 rounded-xl flex items-center justify-center shadow-lg shadow-emerald-500/20">
            <span class="text-white font-black text-xl">R</span>
          </div>
          <span class="text-xl font-bold tracking-tight text-white hidden sm:block">RedYMercadeo</span>
        </div>
        
        <div class="flex items-center gap-4">
          <button @click="openLoginModal" class="text-sm font-medium hover:text-emerald-400 transition-colors">Ingresar</button>
          <button @click="openRegisterModal" class="bg-emerald-500 hover:bg-emerald-400 text-[#070b14] px-5 py-2.5 rounded-full text-sm font-bold shadow-lg shadow-emerald-500/20 transition-all active:scale-95">Unirse Ahora</button>
        </div>
      </div>
    </header>
    ...
    <main class="pt-32 pb-20">
      ...
      <section class="max-w-7xl mx-auto px-6 text-center">
        <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-bold mb-8 animate-fade-in">
        ...
        <h1 class="text-5xl md:text-7xl lg:text-8xl font-black mb-6 leading-tight tracking-tight text-white">
          Tu tiempo es oro, <br/>
        ...
        <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
          <button @click="openRegisterModal" class="w-full sm:w-auto px-8 py-4 bg-emerald-500 hover:bg-emerald-400 text-[#070b14] font-black text-lg rounded-2xl shadow-xl shadow-emerald-500/20 transition-all hover:-translate-y-1 block">
            EMPEZAR A GANAR
          </button>
      ...
      <!-- Stats -->
      <section class="max-w-7xl mx-auto px-6 mt-32 grid grid-cols-2 md:grid-cols-4 gap-6">
        <div v-for="stat in stats" :key="stat.label" class="p-6 bg-white/5 border border-white/10 rounded-3xl backdrop-blur-sm">
    ...
    <transition name="modal">
      <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
        <div @click.stop class="w-full max-w-md bg-[#0d121f] border border-white/10 rounded-[3rem] p-8 relative shadow-2xl">
          <button @click="showModal = false" class="absolute top-6 right-6 text-gray-500 hover:text-white transition-colors duration-200">
          ...
          <div class="text-center mb-10 pt-4">
            ...
            <h2 class="text-2xl md:text-3xl font-black text-white px-4 leading-tight">
              {{ isLogin ? 'Bienvenido de nuevo' : 'Crea tu perfil y empieza a ganar' }}
            </h2>
          </div>

          <!-- Tabs -->
          <div class="flex p-1 bg-white/5 rounded-2xl mb-8 border border-white/5">
            <button type="button" @click="isLogin = true" :class="isLogin ? 'bg-white/10 text-white shadow-lg ring-1 ring-white/10' : 'text-gray-500 hover:text-gray-300'" 
                    class="flex-1 py-3 text-sm font-bold rounded-xl transition-all">Login</button>
            <button type="button" @click="isLogin = false" :class="!isLogin ? 'bg-white/10 text-white shadow-lg ring-1 ring-white/10' : 'text-gray-500 hover:text-gray-300'" 
                    class="flex-1 py-3 text-sm font-bold rounded-xl transition-all">Registro</button>
          </div>

          <form @submit.prevent="handleAuth" class="space-y-4 max-h-[60vh] overflow-y-auto px-2 custom-scrollbar">
            <div v-if="!isLogin" class="grid grid-cols-2 gap-4">
              <div class="col-span-1">
                <label class="text-[10px] font-bold text-gray-500 uppercase tracking-widest ml-1 mb-1 block">Nombre Full</label>
                <input v-model="form.nombre" type="text" placeholder="Tu nombre" 
                       class="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3 text-sm focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none transition-all text-white placeholder:text-gray-600" required>
              </div>
              <div class="col-span-1">
                <label class="text-[10px] font-bold text-gray-500 uppercase tracking-widest ml-1 mb-1 block">Teléfono</label>
                <input v-model="form.telefono" type="tel" placeholder="9988-7766" 
                       class="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3 text-sm focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none transition-all text-white placeholder:text-gray-600" required>
              </div>
            </div>
            ...
            <div>
              <label class="text-[10px] font-bold text-gray-500 uppercase tracking-widest ml-1 mb-1 block">Identidad (Login)</label>
            ...
            <button type="submit" :disabled="loading" 
                    class="w-full bg-gradient-to-r from-emerald-500 to-teal-500 py-4 rounded-2xl text-[#070b14] font-black tracking-wide shadow-xl shadow-emerald-500/20 active:scale-95 transition-all flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed">
              <span v-if="!loading">{{ isLogin ? 'INGRESAR AL SISTEMA' : 'CREAR MI PERFIL YA' }}</span>
          ...
          <p v-if="errorMsg" class="mt-4 text-xs text-red-400 font-bold text-center bg-red-400/10 py-2 rounded-xl border border-red-400/20 animate-shake">{{ errorMsg }}</p>
        </div>
      </div>
    </transition>
...
const handleAuth = async () => {
  ...
  if (isLogin.value) {
    ...
    if (res.success) {
      ...
      let dashboardPath = '/cliente/DashboardCliente'
      
      if (['admin', 'sa'].includes(userRole)) dashboardPath = '/admin/DashboardAdmin'
      else if (userRole === 'tecnico') dashboardPath = '/tecnico/DashboardTecnico'
